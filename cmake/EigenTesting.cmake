...
macro(ei_add_test_internal_coriander testname testname_with_suffix)
  set(targetname ${testname_with_suffix})

  set(filename ${testname}.cu)
  ...
  if(EIGEN_ADD_TEST_FILENAME_EXTENSION STREQUAL cu)
      set_source_files_properties(${filename} PROPERTIES LANGUAGE CXX)
      if (${ARGC} GREATER 2)
        message("argc greater 2 add_executable ${targetname} ${filename}")
        cocl_add_executable(${targetname} ${filename})
      else()
        message("not argc greater 2 add_executable ${targetname} ${filename} OPTIONS ${ARGV2}")
        cocl_add_executable(${targetname} ${filename} OPTIONS ${ARGV2})
      endif()
      target_link_libraries(${targetname} ${Coriander_LIBRARIES})
  else()
    message("not cu add_executable ${targetname} ${filename}")
    cocl_add_executable(${targetname} ${filename})
  endif()
...
endmacro(ei_add_test_internal_coriander)
...
macro(ei_add_test_coriander testname)
  get_property(EIGEN_TESTS_LIST GLOBAL PROPERTY EIGEN_TESTS_LIST)
  set(EIGEN_TESTS_LIST "${EIGEN_TESTS_LIST}${testname}\n")
  set_property(GLOBAL PROPERTY EIGEN_TESTS_LIST "${EIGEN_TESTS_LIST}")
  ...
  if(EIGEN_SPLIT_LARGE_TESTS AND suffixes)
    add_custom_target(${testname})
    foreach(suffix ${suffixes})
      ei_add_test_internal_coriander(${testname} ${testname}_${suffix}
        "${ARGV1} -DEIGEN_TEST_PART_${suffix}=1" "${ARGV2}")
      add_dependencies(${testname} ${testname}_${suffix})
    endforeach(suffix)
  else(EIGEN_SPLIT_LARGE_TESTS AND suffixes)
    ...
    foreach(suffix ${suffixes})
      set(symbols_to_enable_all_parts
        "${symbols_to_enable_all_parts} -DEIGEN_TEST_PART_${suffix}=1")
    endforeach(suffix)
    ei_add_test_internal_coriander(${testname} ${testname} "${ARGV1} ${symbols_to_enable_all_parts}" "${ARGV2}")
  endif(EIGEN_SPLIT_LARGE_TESTS AND suffixes)
endmacro(ei_add_test_coriander)
