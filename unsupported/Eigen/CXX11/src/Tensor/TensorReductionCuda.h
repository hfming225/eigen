...
#if defined(EIGEN_USE_GPU) && defined(__CUDACC__) && !defined(__CORIANDERCC__)
...
struct InnerReductionLauncher<
  ...
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output, typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals) {
    ...
    if (num_blocks > 1) {
      ...
      // won't be a race conditions between multiple thread blocks.
      const int dyn_blocks = divup<int>(num_preserved_vals, DEFAULT_BLOCK_SIZE);
      const int max_blocks = device.getNumCudaMultiProcessors() *
                           device.maxCudaThreadsPerMultiProcessor() / DEFAULT_BLOCK_SIZE;
      const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
      LAUNCH_CUDA_KERNEL((ReductionInitKernel<OutputType, Index>),
                         num_blocks, DEFAULT_BLOCK_SIZE, 0, device, reducer.initialize(),
